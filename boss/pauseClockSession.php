...
else
{
    ...
    if( $cn->numRows($sqlNewTime) > 0 )
    {
        ...
        //echo "in CurrentHour-".$time."-RowTime-".$rowNewTime['start_time']."-time-".strtotime($time)."start_time".strtotime($rowNewTime['start_time'])."-DIFF-".$diff;
    }
    ...
    $sql2 = "UPDATE tbl_user_login_log SET end_time = '".$time."', `user_login_log_status` = 0 WHERE user_login_log_status = 1 AND user_login_id = (SELECT user_login_id FROM tbl_user_login WHERE  DATEDIFF(user_login_date, CURDATE()) = 0 AND user_id = ".$user_id.") ";  
