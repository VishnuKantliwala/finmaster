    ...
    <div id="wrapper">
        ...
        <div class="content-page">
            ...
            <script>
                ...
                function getRecords(payment,from_date,end_date)
                {
                    counter++;
                    $.ajax({  
                        url:"getAllTypeRecords.php?type=getServiceConfirmationRecords",  
                        method:"POST",  
                        data:"payment="+payment+"&from_date="+from_date+"&end_date="+end_date,  
                        success:function(data){  
                            //alert(data);
                            if(data != "false")
                            {
                                ...
                                for(i=0;i<length;i++)
                                {
                                    ...
                                    "<td >"+data[i].service_confirmation_id+"</td>"+
                                    "<td >"+data[i].entry_date+"</td>"+
                                    "<td >"+data[i].total_amount+"</td>"+
                                    "<td >"+data[i].received_amount+"</td>"+
                                    "<td>"+data[i].credit_amount+"</td>"+
                                    "<td >"+data[i].attendant_name+"</td>"+
                                    "<td >"+data[i].user_name+"</td>"+
                                    "<td ><a href='serviceConfirmationUpdate.php?service_confirmation_no="+data[i].service_confirmation_id+"'><div style='border:1px solid #dee2e6;text-align:center;padding:5px;'>Edit <i class='far fa-edit'></i></div></a></td>"+
                                    "<td ><a onClick='deleteRecord("+data[i].service_confirmation_id+")'><div style='border:1px solid #dee2e6;text-align:center;padding:5px;'>Delete <i class='mdi mdi-delete'></i></div></a></td>"+
                                    "</tr>";
                                }
                                row += "</tbody>";
